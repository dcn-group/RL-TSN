...
class NetworkFactory(NetworkFactoryInterface, metaclass=SingletonABC):
    ...
    def parse_node_edge_mac_info(self, graph, flows):
        ...
        mac_list: List[MacAddress] = MAG.MacAddressGenerator.generate_all_unicast_mac_address(graph)
        group_mac_list: List[MacAddress] = MAG.MacAddressGenerator.generate_flow_group_mac_address(flows)
        ...
        flow_mac_dict: Dict[FlowId, MAG.FlowMacMapper] = \
            MAG.MacAddressGenerator.assign_mac_address_to_flow(group_mac_list, flows)
    ...
    def product(self, graph, flows, *args, **kwargs) -> Network:
        ...
        self.parse_node_edge_mac_info(graph, flows)
