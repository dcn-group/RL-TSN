...
class ConfigFileGenerator:
    ...
    @staticmethod
    def generate_ned_file(tsn_network: TSNNetwork = None, flows: List[Flow] = None, graph=None, solution_name=None,
        ...
        for tsn_host in tsn_hosts:
            ...
            for port in tsn_host.ports:
                ...
                peer_node_id: NodeId = NodeId(graph.edge_mapper[forward_edge_id].out_node.node_id)
                backward_edge_id: EdgeId = list(filter(
                    lambda eid: graph.edge_mapper[eid].out_node.node_id == tsn_host.device_id and
                                graph.edge_mapper[eid].in_node.node_id == peer_node_id,
                    graph.edge_mapper))[0]
        ...
        for tsn_switch in tsn_switches:
            ...
            for port in tsn_switch.ports:
                ...
                if peer_node_id in hosts_id:
                    continue
                backward_edge_id: EdgeId = list(filter(
                    lambda eid: graph.edge_mapper[eid].out_node.node_id == tsn_switch.device_id and
                                graph.edge_mapper[eid].in_node.node_id == peer_node_id,
                    graph.edge_mapper))[0]
                peer_port_id: PortNo = list(filter(lambda edge_port_pair: edge_port_pair[0] == backward_edge_id,
                                                   node_edge_port_pair_list[peer_node_id]))[0][1]
    ...
    def create_test_scenario(tsn_network: TSNNetwork = None,
                            graph = None, flows = None,
                            node_edge_mac_info: MAG.NodeEdgeMacInfo = None,
                            test_scenario_dirname: str = None,
                            solution_name = None):
        if test_scenario_dirname is not None:
            config.test_scenario_dir = test_scenario_dirname
        if not os.path.exists(config.test_scenario_dir):
            os.makedirs(config.test_scenario_dir)
        test_scenario_dir = config.test_scenario_dir
        # 生成静态配置文件
        ini_file_content: str = ConfigFileGenerator.generate_ini_file(flows=flows)
        ned_file_content: str = ConfigFileGenerator.generate_ned_file(tsn_network=tsn_network,
                                                                      graph=graph, flows=flows, solution_name=solution_name,
                                                                      node_edge_mac_info=node_edge_mac_info)
        flows_flat_file_content: str = ConfigFileGenerator.generate_flows_xml(flows=flows)
        ...
        ned_filename: str = os.path.join(test_scenario_dir, '{}.ned'.format(solution_name.lower()))
        ini_filename: str = os.path.join(test_scenario_dir, '{}.ini'.format(solution_name.lower()))
