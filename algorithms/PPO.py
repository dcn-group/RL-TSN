...
class ActorCriticAgent:
    def __init__(self, args):
        ...
        self.max_train_steps = args.total_steps
    ...
    def select_action(self, state):
        ...
        action_prob = self.actor(state, self.action_set)
        dist = distributions.Categorical(action_prob)
        action = dist.sample()
        x = action.item()
        self.action_set.append(x)
    ...
    def select_argmax_action(self, state):
        ...
        state = torch.as_tensor(state, dtype=torch.float32).to(self.device)
        action_prob = self.actor(state, self.action_set2)
        x = torch.argmax(action_prob).item()
        self.action_set2.append(x)
        if self.current_step2 % 2 == 0:
            self.action_set = []
    ...
    def train(self):
        ...
        if self.args.use_lr_decay:
            self.lr_decay(self.current_step)
        self.buffer.clear()
